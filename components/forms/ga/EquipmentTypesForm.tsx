...
import { submitEquipmentTypes } from "@/actions/ga/equipment-types";

interface EquipmentTypesFormProps {
  onNext?: (data: EquipmentTypesFormData) => void;
  onBack?: () => void;
  initialData?: EquipmentTypesFormData;
  isLoading?: boolean;
}

export function EquipmentTypesForm({
  onNext,
  onBack,
  initialData,
  isLoading = false,
  ...
  const form = useForm<EquipmentTypesFormData>({
    resolver: zodResolver(equipmentTypesSchema),
  ...
  useEffect(() => {
    console.log("EquipmentTypesForm initialData:", initialData);
  }, [initialData]);
